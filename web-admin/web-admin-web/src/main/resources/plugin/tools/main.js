...
layui.config({
  ...
  var setSkin = function(value) {
    ...
    $('.change-mode').on('click', function () {
        ...
        $.ajax({
            url: getCtxPath() + "/menu/changeMode?id=" + id,
            type: "GET",
    ...
    function replaceHtml(obj, buf) {
        if (obj.children != null && obj.children.length > 0 ) {
            ...
            buf.append('<a class="" href="javascript:;"><i aria-hidden="true" class="layui-icon">' + obj.icon + '</i><span>&nbsp;' + obj.name + '</span></a>');
            ...
            for (i in obj) {
                ...
                buf.append('<a href="javascript:;" kit-target data-options="{url:\'' + getCtxProjectPath() + '/' + obj[i].url + '\',icon:\'' + obj[i].icon + '\',title:\'' + obj[i].name + '\',id:\'' + obj[i].id +'\'}">');
                buf.append('<i class="layui-icon"> ' + obj[i].icon + '</i><span>&nbsp;' +  obj[i].name + '</span></a>');
                buf.append('</dd>');
            }
        } else {
            buf.append('<dd class="layui-nav-item">');
            buf.append('<a href="javascript:;" kit-target data-options="{url:\'' + getCtxProjectPath() + '/' + obj.url + '\',icon:\'' + obj.icon + '\',title:\'' + obj.name + '\',id:\'' + obj.id +'\'}">');
            buf.append('<i class="layui-icon"> ' + obj.icon + '</i><span>&nbsp;' +  obj.name + '</span></a>');
            buf.append('</dd>');
        }
    }
  //基本信息
    $('.user-detail').on('click', function () {
        var id = $(this).attr("data-module-id");
        main_detail('基本信息', 'user/updateUser?id=' + id, 700, 450);
    });
     //修改密码  
