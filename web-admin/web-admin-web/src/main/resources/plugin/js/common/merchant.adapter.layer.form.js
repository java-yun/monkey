    ...
    honglu.adapter.layui.init = function (pageConfig) {
        ...
        layui.use(['form', 'laydate', 'upload'], function () {
            ...
            if (pageConfig.form) {
                ...
                honglu.forEach(uploadArray, function (item) {
                    upload.render(item);
                })
                const verify = $.extend({
                    required: function (value) {
                        if (honglu.stringUtil.isEmpty(value)) {
                            return '内容不可为空';
                        }
                    },
                    // value：表单的值、obj：表单的DOM对象
                    range: function (value, obj) {
                        if (!honglu.util.isNumber(value)) {
                            return "请正确输入数字";
                        }
            ...
            form.on('submit(save)', function (data) {
                ...
                honglu.ajax.post(url, field, pageConfig.form.ajaxSuccess);
                return false;
            });
