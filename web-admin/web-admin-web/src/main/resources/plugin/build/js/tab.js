...
//私用对象
    const _tab = {
        ...
        createTabDom: function() {
            ...
            _toolBody.find('li.kit-item').each(function() {
                ...
                $that.off('click').on('click', function() {
                    const layId = that._title.children('li[class=layui-this]').attr('lay-id');
                    switch (target) {
                        case 'refresh': //刷新
                            switch (_config.renderType) {
                                case renderType.page:
                                    const loadIndex = that.load();
                                    const url = that._title.children('li[lay-id=' + layId + ']').data('url');
                                    ...
                                    break;
                                case renderType.iframe:
                                    const item = that._content.children('div[lay-item-id=' + layId + ']').children('iframe');
        ...
        load: function() {
            ...
            // return layer.load(2, {time: 100 , shade: [0.3, '#333'] });
        },
        ...
        winResize: function() {
            ...
            _win.on('resize', function() {
                const currBoxHeight = $(that._parentElem).height(); //获取当前容器的高度
                switch (_config.renderType) {
                    case renderType.page:
        ...
        tabAdd: function(options) {
            ...
            if (_config.openWait)
                loadIndex = that.load();
            const titleHtm = ['<li class="layui-this" lay-id="' + id + '" data-url="' + url + '">'];
            if (icon) {
    ...
    const Tab = function () {
        this.config = {
            ...
            mainUrl: getCtxPath() + '/plugin/html/Welcome.html',
            renderType: 'iframe',
            openWait: true
        };
        this.v = '1.0.5';
